...
func (self *awsSdkMetadata) GetMetaData(key string) ([]byte, error) {
	v, err := aws.GetMetaData("latest/meta-data/" + key)
	if err != nil {
...
func newAWSCloud(config io.Reader, authFunc AuthFunc, metadata AWSMetadata) (*AWSCloud, error) {
	...
	// TODO: wait for this to be merged: https://github.com/awslabs/aws-sdk-go/issues/108
	valid := aws.IsRegionValid(regionName)
	if !valid {
